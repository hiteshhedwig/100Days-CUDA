...
__global__ void matrixTranspose(const float *A, float *C, 
    ...
    __shared__ float sharedTile[TILE_SIZE][TILE_SIZE];

    int bY = blockIdx.y;  int bX = blockIdx.x;
    int thX = threadIdx.x ; int thY = threadIdx.y;

    int row = bY * TILE_SIZE + thY;
    int col = bX * TILE_SIZE + thX;

    // printf("(Row, Col) = (%d,%d) \n", row, col);
    // printf("(bY, bX) - (thY, thX) = (%d,%d) - (%d,%d)\n", bY, bX, thY, thX);


    if ((row < width) && (col<width)) {

        // load elements into the tiles
        sharedTile[thY][thX] = A[row*width+col];
        __syncthreads();

        // how write them transposed into final matrix C
        C[col*width+row] = sharedTile[thY][thX];
        // __syncthreads();
    }
}
...
int main() {
    ...
    std::ofstream resultsFile("benchmark_results_mat_transpose-opt.csv");
    ...
    resultsFile.close();
    printf("Benchmark complete. Results saved to benchmark_results_mat_transpose-opt.csv\n");
