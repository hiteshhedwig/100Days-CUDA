...
def combine_results():
    ...
    cuda_df = pd.read_csv('build/benchmark_results_add.csv')
    ...
    pytorch_df = pd.read_csv('build/pytorch_benchmark_results.csv')
...
def create_execution_time_plot(df):
    ...
    plt.title('Matrix Addition Performance Comparison', fontsize=16)
