...
def benchmark_pytorch(size, iterations=10):
    ...
    for _ in range(iterations):
        c_cpu = a_cpu.t()  # Transpose operation
    ...
    if torch.cuda.is_available():
        ...
        # Warmup
        c_gpu = a_gpu.t()  # Transpose operation
...
def run_pytorch_benchmarks():
    ...
    df.to_csv(f'build/pytorch_benchmark_matrix_transpose_results.csv', index=False)
    print("PyTorch benchmark complete. Results saved to pytorch_benchmark_matrix_transpose_results.csv")
    return df
